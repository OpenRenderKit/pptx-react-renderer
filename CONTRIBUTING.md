...
`pnpm run test:visual` requires LibreOffice (`soffice`) and Poppler (`pdftoppm`) locally. It generates focused edge-case decks and office-rendered references on demand. GitHub Actions runs the same suite for metrics and artifacts on every change, while hard enforcement is reserved for local investigation and future major-release gates. See [docs/TESTING.md](./docs/TESTING.md) for setup details and the current visual-regression model.
